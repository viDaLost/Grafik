...
const els = {
  ...
  // Новые элементы для мобильной навигации и даты
...
function renderAccessMode() {
  ...
  if (els.userBadge) {
    const roleText = isAdmin ? ' (Админ)' : ' (Просмотр)';
    els.userBadge.textContent = getCurrentUserDisplayName() + roleText;
  }
  if (els.accessNote) {
    els.accessNote.textContent = isAdmin
      ? 'Режим администратора: отметки, ученики и история доступны для изменения.'
      : 'Режим просмотра: изменения доступны только админам.';
  }
  ...
  if (els.studentsHelp) {
    els.studentsHelp.textContent = isAdmin
      ? 'Добавляйте новых участников и удаляйте лишних.'
      : 'Список учеников и их счётчики посещений.';
  }
  if (els.historyHelp) {
    els.historyHelp.textContent = isAdmin
      ? 'Здесь показываются все сохранённые даты. Доступно удаление дат.'
      : 'Здесь показываются все сохранённые даты.';
  }
...
function renderAttendance() {
  ...
  els.studentsList.innerHTML = students.map((student) => {
    ...
    const counters = `Б:${student.presentCount || 0}  П:${student.absentCount || 0}`;
    
    // Мобильный переключатель
    const actions = state.canEdit ? `
          <div class="status-pill-control">
            <button class="pill-opt ${!isAbsent ? 'active-present' : ''}" data-student-id="${student.studentId}" data-status="present">Был</button>
            <button class="pill-opt ${isAbsent ? 'active-absent' : ''}" data-student-id="${student.studentId}" data-status="absent">Н/Б</button>
          </div>
        ` : `
          <div class="view-status-pill ${isAbsent ? 'absent' : 'present'}">${isAbsent ? 'Н/Б' : 'Был(а)'}</div>
        `;
      ...
      <article class="attendance-row ${isAbsent ? 'absent' : ''}" data-student-id="${student.studentId}">
        <div class="student-main-layout">
          ...
          <div class="student-info">
            <div class="student-fullname">${escapeHtml(student.fullName)}</div>
            <div class="student-meta-row">
              <span class="stat-counter">${escapeHtml(counters)}</span>
              ${isAbsent ? `<span class="absent-reason-pill" data-edit-reason="${student.studentId}">${escapeHtml(record.reason || 'Указать причину')}</span>` : ''}
            </div>
          </div>
        </div>
...
function renderStudentsAdmin() {
  ...
  els.studentsAdminList.innerHTML = state.students.map((student, index) => `
    <div class="admin-row">
      <div>
        <strong class="admin-student-name">${index + 1}. ${escapeHtml(student.fullName)}</strong>
        <div class="admin-stats-badges">
          <span class="badge-stat green">Был(а): ${student.presentCount || 0}</span>
          <span class="badge-stat red">Пропуски: ${student.absentCount || 0}</span>
        </div>
      </div>
      ${state.canEdit ? `<button class="delete-mini-btn" data-delete-student="${student.studentId}">Удалить</button>` : ''}
    </div>
  `).join('');
}
...
function renderHistory() {
  if (!state.history.length) {
    els.historyList.innerHTML = '<div class="empty-state">История пока пуста.</div>';
    return;
  }
  ...
  els.historyList.innerHTML = state.history.map((row) => `
    <div class="history-row">
      <div class="history-info-block">
        <h4>${formatDateRu(row.date)}</h4>
        ${row.lessonNote ? `<div class="history-note-text">${escapeHtml(row.lessonNote)}</div>` : ''}
      </div>
      <div class="history-bottom-meta">
        <div class="history-counts">
          <span class="badge-stat green">Б: ${row.present}</span>
          <span class="badge-stat red">П: ${row.absent}</span>
        </div>
        <div class="history-actions">
          <button class="history-btn-mini details" data-history-details="${row.date}">Просмотреть</button>
          ${state.canEdit ? `<button class="history-btn-mini delete" data-history-delete="${row.date}">Удалить</button>` : ''}
        </div>
      </div>
    </div>
  `).join('');
...
function openHistoryModal(date) {
  ...
  els.historyModalBody.innerHTML = `
    <div class="history-modal-summary">
      <span class="badge-stat green">Были: ${entry.present}</span>
      <span class="badge-stat red">Пропустили: ${entry.absent}</span>
    </div>
    ${entry.lessonNote ? `<div class="history-modal-note"><strong>Заметка:</strong> <p style="margin: 4px 0 0; font-size:13px;">${escapeHtml(entry.lessonNote)}</p></div>` : ''}
...
// Переключение на день назад/вперед
async function changeDateByOffset(offset) {
  const currentDate = new Date(`${state.date}T00:00:00`);
  currentDate.setDate(currentDate.getDate() + offset);
  
  const tzOffset = currentDate.getTimezoneOffset() * 60000;
  const newIso = new Date(currentDate.getTime() - tzOffset).toISOString().slice(0, 10);
  
  state.date = newIso;
  els.lessonDate.value = newIso;
...
function bindUI() {
  ...
  els.prevDateBtn.addEventListener('click', () => changeDateByOffset(-1));
  els.nextDateBtn.addEventListener('click', () => changeDateByOffset(1));

  // Кнопка показа информации/прав
  els.infoBtn.addEventListener('click', () => {
    els.accessNote.classList.toggle('hidden');
  });
